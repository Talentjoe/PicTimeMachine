...
| 组件 | 职责 | 触发条件 |
| --- | --- | --- |
| `MarkerClusterLayer` | 聚类渲染全部可见标记；`highlight` 时把最后一张单独画成红色大图标并自动开 popup | 始终渲染 |
| `FitBounds` | 把视口框定到全部可见标记 | "显示全部"状态（`showAll`） |
| `FocusOnMarkers` | 播放时平移/缩放到当前图片（单点居中带偏移，多点 fitBounds） | 播放状态（非 `showAll`） |

`showAll` / `highlight` 由父组件 `features/photo-track/PhotoTrackPage.tsx` 根据时间轴位置计算（时间轴是图片索引，语义见 `features/timeline`）。
